      ...
      <nav className="bg-white border-b border-gray-200 sticky top-0 z-50">
          ...
          <div className="flex justify-between items-center h-16">
              ...
              {menuItems.map(item => (
                <a
                  ...
                  className={`text-sm font-medium ${
                    navSelect === item.name ? 'text-tomato border-b-2 border-tomato' : 'text-gray-600'
                  }`}
                >
                  {item.label}
                </a>
            ...
            <div className="flex items-center space-x-4">
              ...
              <Link to="/cart" className="relative">
                <ShoppingBag size={20} />
                <span className="absolute -top-1 -right-1 bg-tomato text-white text-[10px] rounded-full px-1">3</span>
              </Link>
              ...
              {isLogin ? (
                <Menu as="div" className="hidden md:block relative">
                  ...
                  <MenuItems className="absolute right-0 mt-2 w-40 bg-white shadow rounded">
                    <MenuItem>
                      <Link to='/myDashboard' className="block px-4 py-2 hover:bg-gray-100">Dashboard</Link>
                    </MenuItem>
                    <MenuItem>
                ...
                <button onClick={() => setModal(true)} className="hidden md:block border px-3 py-1 rounded-full hover:bg-tomato hover:text-white">
                  Sign In
                </button>
              )}
        ...
        {isOpen && (
          <div className="md:hidden bg-white px-4 py-4 space-y-2">
            {menuItems.map(item => (
              <a key={item.name} href={item.to} onClick={() => setIsOpen(false)} className="block py-2 text-gray-600">
                {item.label}
              </a>
            ))}
            {isLogin ? (
              <button onClick={handleLogout} className="flex items-center gap-2 py-2 text-tomato"><LogOut size={18} /> Sign Out</button>
            ) : (
              <button onClick={() => setModal(true)} className="block py-2">Sign In</button>
            )}
          </div>
        )}
      </nav>
